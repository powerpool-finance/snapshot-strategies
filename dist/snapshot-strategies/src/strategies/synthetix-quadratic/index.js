...
async function strategy(_space, _network, _provider, _addresses, _, snapshot) {
    ...
    if (l1Results && l1Results.snxholders) {
        for (let i = 0; i < l1Results.snxholders.length; i++) {
            ...
            const weightedVoteL1 = await (0, helper_1.debtL1)(holder.initialDebtOwnership, holder.debtEntryAtIndex, totalL1Debt, scaledTotalL2Debt, lastDebtLedgerEntry, true);
            score[(0, address_1.getAddress)(holder.id)] = weightedVoteL1;
        }
    }
    ...
    */
    if (l2Results && l2Results.snxholders) {
        for (let i = 0; i < l2Results.snxholders.length; i++) {
            ...
            const weightedVoteL2 = await (0, helper_1.debtL2)(holder.initialDebtOwnership, holder.debtEntryAtIndex, totalL1Debt, scaledTotalL2Debt, lastDebtLedgerEntryL2, true);
            if (score[(0, address_1.getAddress)(holder.id)]) {
                score[(0, address_1.getAddress)(holder.id)] += weightedVoteL2;
            }
            else {
                score[(0, address_1.getAddress)(holder.id)] = weightedVoteL2;
            }
