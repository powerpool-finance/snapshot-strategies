...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    // Fetch FXS Balance
    const fxsQuery = addresses.map((address) => [
        options.FXS,
        'balanceOf',
        [address]
    ]);
    // Fetch veFXS Balance
    const vefxsQuery = addresses.map((address) => [
        options.VEFXS,
        'balanceOf',
        [address]
    ]);
    // Fetch FREE_UNI_LP_FRAX_FXS Balance
    const freeUniLPFraxFxsQuery = addresses.map((address) => [
        options.UNI_LP_FRAX_FXS,
        'balanceOf',
        [address]
    ]);
    // Fetch FARMING_UNI_LP_FRAX_FXS Balance
    const farmingUniLPFraxFxsQuery = addresses.map((address) => [
    ...
    return Object.fromEntries(Array(addresses.length)
        ...
        return [
            addresses[i],
            parseFloat((0, units_1.formatUnits)(free_fxs
                .add(vefxs)
