...
async function getPairs(network, snapshot, token) {
    ...
    const [result1, result2] = await Promise.all(['token0', 'token1'].map((prop) => (0, utils_1.subgraphRequest)(SUSHISWAP_SUBGRAPH_URL[network].exchange, getParams(prop))));
    return Object.fromEntries(result1.pairs.concat(result2.pairs).map((pair) => {
...
async function getPools(network, snapshot, token) {
    ...
    const result = await (0, utils_1.subgraphRequest)(SUSHISWAP_SUBGRAPH_URL[network].masterChef, params);
    return Object.fromEntries(result.pools.map((pool) => [pool.id, pairs[pool.pair]]));
}
async function getStakedBalances(network, snapshot, token, addresses) {
    ...
    return result.users.map((user) => {
        ...
        const amount = parseFloat((0, units_1.formatUnits)(user.amount, pool.decimals)) * pool.rate;
        return {
...
async function strategy(_space, network, _provider, addresses, options, snapshot) {
    ...
    const result = await (0, utils_1.subgraphRequest)(SUSHISWAP_SUBGRAPH_URL[network].exchange, params);
    const score = {};
    ...
    if (options.useStakedBalances === 'true') {
        const stakedBalances = await getStakedBalances(network, snapshot, options.address, addresses);
        stakedBalances.forEach((balance) => {
            const userAddress = (0, address_1.getAddress)(balance.address);
            if (!score[userAddress])
                score[userAddress] = 0;
