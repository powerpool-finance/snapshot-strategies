...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    while (nodes.length < 10000) {
        ...
        const result = await (0, utils_1.subgraphRequest)(RENVM_SUBGRAPH_URL[network], RENVM_SUBGRAPH_QUERY);
    ...
    const scores = addresses.reduce((obj, address) => ({ ...obj, [(0, address_1.getAddress)(address)]: 0 }), {});
    nodes.forEach((darknode) => {
        // Skip operators that aren't in `addresses`.
        const nodeOperator = (0, address_1.getAddress)(darknode.operator);
