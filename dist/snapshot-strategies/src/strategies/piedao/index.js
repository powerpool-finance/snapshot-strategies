...
async function strategy(space, network, provider, addresses, options, snapshot) {
    ...
    const doughv1Query = addresses.map((address) => [
        options.doughv1,
        'balanceOf',
        [address]
    ]);
    ...
    const stakedDoughQuery = addresses.map((address) => [
        options.stakedDough,
        'balanceOf',
        [address]
    ]);
    const lpDoughQuery = addresses.map((address) => [
        options.BPT,
        'balanceOf',
        [address]
    ]);
    ...
    return Object.fromEntries(Array(addresses.length)
        ...
        .map((_, i) => [
        addresses[i],
        parseFloat((0, units_1.formatUnits)(doughv2BPT[0]
